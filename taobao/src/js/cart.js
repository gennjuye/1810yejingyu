define(['config'], function() {
	require(['jquery', 'details'], function() {
		...
		//全-----------------选
		(function(){
			//console.log($('.bod-check').find('input').prop('checked',true))
		...
		//计算商品总价
		function concart(obj){
				var checked = obj.parents('.bod-list').find(':checkbox')
				var price=0;
				$.each(checked,function(index,value){
					if($(value).prop('checked')){
						price+=Number($(value).parents('.box').find('.zpirce i').html());
						$('.bod-total-right em').html(price/2);
					}else{
						$('.bod-total-right em').html('￥ 0.00');
					}
				})
